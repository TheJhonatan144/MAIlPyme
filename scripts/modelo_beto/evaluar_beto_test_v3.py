...
def main() -> None:
    ...
    metrics_json_path = (
        metrics_dir / "metricas_test_beto_v3.json"
    )
    metrics_csv_path = (
        metrics_dir / "metricas_test_beto_v3.csv"
    )
    report_path = (
        metrics_dir
        / "reporte_clasificacion_test_beto_v3.csv"
    )
    matrix_csv_path = (
        metrics_dir
        / "matriz_confusion_test_beto_v3.csv"
    )
    predictions_path = (
        metrics_dir
        / "predicciones_test_beto_v3.csv"
    )
    errors_path = (
        metrics_dir
        / "errores_test_beto_v3.csv"
    )
    matrix_plot_path = (
        plots_dir
        / "matriz_confusion_test_beto_v3.png"
    )
    ...
    axis.set_title(
        "Matriz de confusión - BETO MailPyme AI v3"
    )
    figure.tight_layout()
    figure.savefig(
        matrix_plot_path,
